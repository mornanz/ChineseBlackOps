...
class CameraStub:
    # Prosta "atrapa" kamery z ISO tylko w sofcie.
    def __init__(self):
...
class AutoAdjust:
    # Real-time capture, histogram i detekcja emocji (YOLO na twarzy).
    def __init__(self, queue: asyncio.Queue, camera, save_frames=False, display_frames=True):
        ...
        self.display_interval = 0.2  # ~5 FPS
        ...
        print("[INIT] YOLO classes:")
        for i, name in self.yolo.names.items():
            print(f"  {i}: {name}")
...
class HistogramAnalyzer:
    # ISO optimizer używający histogramu.
